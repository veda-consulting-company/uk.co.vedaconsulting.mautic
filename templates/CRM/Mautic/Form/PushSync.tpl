<div class="crm-block crm-form-block crm-campaignmonitor-sync-form-block">
  {if $smarty.get.state eq 'done'}
    <div class="help">
      ...
      <tr><td>{ts}Contacts on Mautic (originally){/ts}:</td><td>{$group.stats.mc_count}</td></tr>
      ...
      <tr><td>{ts}Contacts Unsubscribed from Mautic{/ts}:</td><td>{$group.stats.unsubscribes}</td></tr>
      </table>
      {/foreach}
    </div>
    {if $error_messages}
